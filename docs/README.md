...
To deploy a new server (Ubuntu), use `ssh-keygen` to generate an SSH keypair. Add the public key as a deploy key to the GitHub repository, then use the [provided install script](_media/setup.sh) or CURL command below to run setup:

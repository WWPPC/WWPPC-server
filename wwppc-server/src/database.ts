...
export class Database {
    ...
    async readRounds(c: ReadRoundsCriteria): Promise<Round[]> {
        const startTime = performance.now();
        try {
            const data = await this.#db.query(`SELECT * FROM rounds WHERE ${c.contest != undefined ? 'contest=$1' : '1=1'}${(c.contest != undefined && c.round != undefined) ? ' AND ' : ''}${c.round != undefined ? 'number=$2' : '1=1'}`, [c.contest, c.round]);
